  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        // BẠN BỊ THIẾU VIỆC LẤY completedLessons TỪ API Ở ĐÂY
        const { data, access, completedLessons } = response.data;
        
        setCourse(data);
        setAccessMode(access);
        setCompletedLessons(completedLessons || []); // Lưu vào state
            ...
            <div key={unit.id} className="mb-4">
              ...
              {unit.items.map(item => (
                <div 
                  ...
                  className={`px-4 py-3 text-sm cursor-pointer rounded-xl transition-all mb-1 flex items-center gap-3 ${activeItem?.id === item.id ? 'bg-blue-50 text-blue-700 font-bold shadow-sm ring-1 ring-blue-200' : 'hover:bg-slate-50 text-slate-600'}`}
                >
                  {/* BẠN BỊ THIẾU LOGIC ĐỔI ICON Ở ĐÂY */}
