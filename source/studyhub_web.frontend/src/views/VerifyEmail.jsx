...
export default function VerifyEmail({ email, onSuccess, onBack }) {
  ...
  const handleVerify = async () => {
    ...
    setError('');
    try {
      const { data } = await CourseAPI.verifyEmail(email, code);
  ...
  const handleResend = async () => {
    ...
    setSuccessMsg('');
    try {
      await CourseAPI.resendOtp(email);
      setSuccessMsg('Mã mới đã được gửi!');
      setResendCooldown(60);
      setOtp(['', '', '', '', '', '']);
      inputs.current[0]?.focus();
    } catch (error) {
      setError(error.response?.data?.message || 'Gửi lại thất bại!');
    } finally {
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen overflow-hidden bg-slate-900 font-sans">
      ...
      <div className="relative z-10 w-full max-w-md p-10 mx-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
        ...
        {error && <div className="p-3 mb-5 text-sm text-center text-red-200 bg-red-500/20 rounded-xl border border-red-500/20">{error}</div>}
        {successMsg && <div className="p-3 mb-5 text-sm text-center text-green-200 bg-green-500/20 rounded-xl border border-green-500/20">{successMsg}</div>}
        ...
        <button onClick={onBack} className="flex items-center justify-center gap-1 w-full mt-5 text-sm text-slate-500 hover:text-slate-300 transition-colors">
