...
export default function CoursePricingEditor({
  ...
  // Hàm Validate (Đã được phục hồi và làm gọn)
  const validate = (val) => {
  ...
  const dNum = (isDiscountEnabled && discountInput !== '') ? Number(discountInput) : pNum;
  const finalPrice = dNum < pNum ? dNum : pNum;
  const isFree = finalPrice === 0;

  // Tính phần trăm giảm giá (Đã được phục hồi)
  const savingAmount = pNum - finalPrice;
  const savingPercent = pNum > 0 && savingAmount > 0 ? Math.round((savingAmount / pNum) * 100) : 0;
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-100">
        ...
        {isFree ? (
          ...
          isDiscountEnabled && savingPercent > 0 && (
      ...
      <div className="p-4 space-y-3">
        ...
        <button
          onClick={handleSave}
          disabled={isSaving || !!error || (isDiscountEnabled && discountInput === '')}
